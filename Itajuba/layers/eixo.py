...
def eixos(cur):
    ...
    # Variável para contar quantos eixos não têm logradouro correspondente.
    nao_encontrado = 0
    ...
    for eixo in eixos:
        ...
        if logName:
            ...
            print(f'Logradouro não encontrado para eixo {eixo[1]}, aplicando buffer de 0.5 metros.')

            # Converter a geometria WKB para Shapely para aplicar o buffer.
            eixo_geom = wkb.loads(bytes(eixo[0]))
            eixo_geom_com_buffer = eixo_geom.buffer(0.5)

            # Nova tentativa com o eixo bufferizado.
            cur.execute("""
                SELECT l.id, l.nome, ST_Length(ST_Intersection(l.geom, ST_GeomFromText(%s, 31983))) / ST_Length(l.geom) AS overlap
                FROM dado_novo.logradouro l
                WHERE ST_Intersects(l.geom, ST_GeomFromText(%s, 31983))
                ORDER BY overlap DESC LIMIT 1
            """, (eixo_geom_com_buffer.wkt, eixo_geom_com_buffer.wkt))
            ...
            if logName_buffer:
                ...
                # Caso não encontre nem com o buffer, insere sem logradouro associado.
                nao_encontrado += 1
                if nao_encontrado == 1:
    ...
    print(f"Processo de inserção de eixos finalizado. Eixos sem logradouro: {nao_encontrado}")
    print("----------Fim eixos-----------------")
